 ...
 *
 * Unidirectional mappings from Italian STEMS to English STEMS.
 * Used to translate Italian task terms to find English code.
 *
 * Direction: IT → EN only (not bidirectional)
 *
 * Benefits:
 * - One entry covers all morphological variants
 * - "utente", "utenti" all stem to "utent" → maps to "user"
 * - Allows finding English code when task is in Italian
 *
...
export const stemTranslations: [string, string][] = [
  ...
  ['antepr', 'preview'], // anteprima → preview
  ['sfoglia', 'brows'], // sfogliare → browse
  ['condivid', 'shar'], // condividere → share
  ['test', 'test'],
  ...
  // === Moderation ===
  ['segnal', 'report'], // segnalazione, segnalare → report (stem is 'segnal' not 'segnalaz')
  ...
  // === Services & Features ===
  ['serviz', 'servic'], // servizio, servizi → service
  ['personagg', 'charact'], // personaggio, personaggi → character

  // === Restrictions ===
  ['restriz', 'restrict'], // restrizione → restriction

  // === Words/Content ===
  ['parol', 'word'], // parola, parole → word
  ['banner', 'banner'], // banner → banner (identity for compound matching)
];
